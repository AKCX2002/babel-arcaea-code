...
class Compat {
    private array $opts;
    public function __construct() { $this->opts = Plugin::init()->options()->get(); if(empty($this->opts['enabled']))return; if(!empty($this->opts['disable_sakurairo_prism']))\add_action('wp_enqueue_scripts',[$this,'disablePrism'],999); if(!empty($this->opts['aplayer_safe_patch']))\add_action('wp_head',[$this,'aplayerPatch'],0); if(!empty($this->opts['suppress_lightgallery_warn']))\add_action('wp_head',[$this,'lgSuppress'],999); }
