...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), function ($links) {
    array_unshift(
        $links,
        '<a href="' . esc_url(admin_url('options-general.php?page=babel-arcaea-code')) . '">设置</a>'
    );
    return $links;
});
...
add_action('wp_enqueue_scripts', function () {
    ...
    if ($deps && !in_array('bac-medium-zoom', $deps->deps, true)) {
